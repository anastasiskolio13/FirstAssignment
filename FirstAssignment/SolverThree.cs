...
namespace FirstAssessment
{
    class SolverThree : Solver<string>
    {
        ...
        List<Dictionary<char,int>> createHashTable()
        {
            List<Dictionary<char, int>> H = [];
            for (int i = 0; i < A.Count; ++i)
            {
                H.Add(new Dictionary<char, int>());
                foreach(char character in A[i])
                {
                    if (H[i].ContainsKey(character))
                    {
                        H[i][character] += 1;
                    }
                    else
        ...
        bool IsAnagram(int i, List<Dictionary<char, int>> H)
        {
